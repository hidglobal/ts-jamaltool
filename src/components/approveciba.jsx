  ...
  const form49 = useForm({
    ...
    validate: (values) => {
      ...
      if (active === 0) {
        return {
          userid:
            values.userid.trim().length < 2 ? "Please enter a username" : null,
          deviceid:
            sessionStorage.getItem("deviceid").length < 2
              ? "Please choose a device id"
              : null,
        };
      }
  ...
  return (
    <Grid>
      ...
      <Grid.Col span={8}>
        ...
        >
          <Center>
            <h3>Test HID Approve Push Authentication.</h3>
          </Center>
          ...
          <Stepper active={active} breakpoint="sm">
                        ...
                        axios
                          ...
                          .then(function (response) {
                            ...
                            if (response.data.resources != null) {
                              ...
                              const userinternal =
                                response?.data?.resources["0"]?.owner.value;
                              sessionStorage.setItem(
                                "userInternalId",
                                userinternal
                              );
                              const device_data = devices.map((device) => (
                                <option key={device.id} value={device.id}>
                                  {device.friendlyName ||
                                    "Device is not provisioned"}
                                </option>
                              ));
                              let devicehtm = renderToString(
                                <Input
                                  component="select"
                                  rightSection={
                                  ...
                                  value={pvalue}
                                  onChange={setValue(
                                    document.getElementById("scdevice")?.value
                                  )}
                                  icon={<IconDeviceMobile />}
                                  radius="xl"
                                  size="sm"
                                  {...form49.getInputProps("deviceid")}
                                >
                                  {device_data}
                                </Input>
                              );
                              ...
                              document.getElementById(
                                ...
                                "Your user internal ID: " + userinternal;
                            }
                          });
                      }
                    }}
                ...
                {document.getElementById("scdevice")?.value?.length > 2 && (
                  <Center>
                    <Button
                      ...
                      onClick={() => {
                        if (
                          document.getElementById("scdevice")?.value != null
                        ) {
                          const deviceName =
                            document.getElementById("scdevice")?.options[
                              document.getElementById("scdevice").selectedIndex
                            ].text;
                          const deviceID =
                            document.getElementById("scdevice")?.value;
                          document.getElementById("currentDevice").innerHTML =
                            "Current selected device: " + deviceName;
                          document.getElementById("currentDevice").innerHTML +=
                            "<br/>Current selected device id: " + deviceID;
            ...
            <Stepper.Step
              label="Craft ID Token"
              description="ID Token Settings"
            >
              <Center>
                <Chip defaultChecked color="indigo" variant="filled">
                  You have selected this device to send Push Notification:{" "}
              ...
              <Center>
                <h4>
                  Craft Header and Body of the Push Notification. ( Generate ID
                  Token )
                </h4>
              </Center>
              ...
              <Center>
                <Button
                  onClick={() => {
                    ...
                    form49.setFieldValue(
                      "bcPayload",
                      '{  "scope": "openid hid-tx-sign",  \n    "client_notification_token": "8d67dc78-7faa-4d41-aabd-67707b374255", \n     "acr_values": "mod-mf", \n     "login_hint_token": "' +
                        CIBAtokenLogin +
                        '"\n  }'
                    );
                    document.getElementById("gToken").innerText =
                      "Generated Token:\n" + CIBAtokenLogin;

                    const messagesuccess = renderToString(
                      <Center>
                        <Chip defaultChecked color="teal" variant="filled">
                          Successfully crafted an ID Token.
                        </Chip>
                      </Center>
                    );
                            ...
                            <>
                              <Center>
                                <Notification
                                  ...
                                  color="teal"
                                >
                                  <Loader color="teal" size={12} /> Please wait
                                  until we recieve verification from HID Approve
                                </Notification>
                              </Center>
                            </>
                          );
                          ...
                          const ErrorNot = renderToString(
                            <>
                              <Center>
                                ...
                                >
                                  There's an error when we tried to send the
                                  Push Notification.
                                </Notification>
                              </Center>
                            </>
